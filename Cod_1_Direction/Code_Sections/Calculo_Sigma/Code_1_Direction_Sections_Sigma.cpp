...
void LugaresEmisionAleatorios()
{
    ...
    ofstream myfileProbSigma;
    myfileProbSigma.open("Eventos_ProbabilidadEscape_Sigma.txt");
    //---------------------------------
    int NumeroEventos;
    ...
    int NumeroPuntosGrafica = 10;
    double PowerMin = 2, PowerMax = 5;
    int NumeroPuntosPromedio = 20;
    double ProbabilidadOut[NumeroPuntosPromedio];
    ...
    double deltaPower = (double)(PowerMax-PowerMin)/(NumeroPuntosGrafica-1);
    cout<<"\n Delta Power: "<<deltaPower;
    
    for(k=0; k<NumeroPuntosGrafica; k++)
    {
        NumeroEventos = (int)pow(10,PowerMin+deltaPower*k);
        
        cout<<"\n k: "<<k;
        cout<<"\t No. eventos: "<<NumeroEventos<<"\n";
        
        l=0;
        srand((int)time(0));
        mean = 0;
        sigma = 0;
        while(l<NumeroPuntosPromedio)
        {
        
            NumeroEventosDetector = 0;
            CaminoAntes = 0;
            CaminoMismoMaterial = 0;
            ...
            ProbEscape = ((double)NumeroEventosDetector)*100/((double)NumeroEventos);
            /*
            cout<<"\n Valor k: "<<k;
            cout<<"\n De "<<NumeroEventos<<" eventos, "<<NumeroEventosDetector;
            cout<<" se detuvieron en detector. \tPorcentaje: ";
            cout<<fixed<<setprecision(2)<<ProbEscape;
            */
            ProbabilidadOut[l] =ProbEscape;
            l++;
        }
        
        for(int ll=0; ll<NumeroPuntosPromedio; ll++)
        {
            mean += ProbabilidadOut[ll]/NumeroPuntosPromedio;
        }
        

        for(int ll=0; ll<NumeroPuntosPromedio; ll++)
        {
            sigma += pow(mean - ProbabilidadOut[ll],2);
        }
        d = sigma;
        sigma = sqrt(d);
        
        cout<<"\n Eventos: "<<NumeroEventos;
        cout<<"\tPromedio: "<<fixed<<setprecision(4)<<mean;
        cout<<"\tSigma: "<<fixed<<setprecision(4)<<sigma;
        
        myfileProbSigma<<fixed<<setprecision(4)<<NumeroEventos<<"\t"<<mean<<"\t"<<sigma<<"\n";
